...
class Edge:
    ...
    # t - точка в одномерном пространстве
    def r3(self, t):
...
class Polyedr:
    ...
    def __init__(self, file):
        ...
        self.perimetr = 0.0  # искомая величина
    ...
    def per(self):  # поиск искомой величины
        for e in self.edges:
            for f in self.facets:
                e.shadow(f)
            if (not e.gaps and abs(2 * self.c - e.r3(
                    0.5).proection_x()) < self.c  # условие что ребро невидимо и что центр ребра в нужной "зоне"
                    and acos(abs((e.r3(0) - e.r3(1)).proection_z() /
                                 e.r3(0.0).length(e.r3(1.0)))) > pi / 18):  # условие на угол
                self.perimetr += e.r3(0.0).length(e.r3(1.0)) / self.c
