...
class Polyedr:
    ...
    def per(self):  # поиск искомой величины
        for e in self.edges:
            for f in self.facets:
                e.shadow(f)
            if (not e.gaps and  # условие что ребро невидимо
                    abs(2 * self.c - e.r3(0.5).proection_x())
                    < self.c and  # условие на центр ребра и угол
                    acos(abs((e.r3(0) - e.r3(1)).proection_z() /
                             e.r3(0.0).length(e.r3(1.0)))) > pi / 18):
